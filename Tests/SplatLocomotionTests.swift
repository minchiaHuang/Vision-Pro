...
struct SplatLocomotionTests {
    ...
    @Test func manualForwardMovesAlongYawForward() {
        ...
        // speed = span(1) * moveFraction(0.06) * dt(1) = 0.06; forward at yaw 0 = (0,0,-1)
        #expect(abs(loco.position.z - (-0.06)) < 1e-5)
    ...
    @Test func speedScalesWithSpanAndDeltaTime() {
        ...
        // speed = 10 * 0.06 * 0.5 = 0.3
        #expect(abs(loco.position.z - (-0.3)) < 1e-4)
    }
    ...
    @Test func noBoundaryLeavesMovementUnbounded() {
        ...
        // 5 steps * (100 * 0.06) = 30 along -z, no wall to stop it.
        #expect(abs(loco.position.z - (-30)) < 1e-3)
    }
