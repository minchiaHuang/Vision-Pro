...
struct ExitScreen: View {
    ...
    var body: some View {
        ZStack {
            OopsPassthrough()
            ZStack {
                Image("oops_meadow").resizable().scaledToFill()
                Color.black.opacity(0.40)
                VStack(spacing: 28) {
                    Text("You've stepped out")
                        .font(.system(size: 40, weight: .bold))
                        .foregroundStyle(.white)
                    Text("Your world is paused right here, waiting whenever you'd like to return.")
                        .oopsSub(22)
                        .foregroundStyle(.white.opacity(0.85))
                        .multilineTextAlignment(.center)
                        .frame(maxWidth: 620)
                    HStack(spacing: 28) {
                        Button("Re-enter", action: onReenter).buttonStyle(OopsButton())
                        Button("Back to Home", action: onHome).buttonStyle(OopsButton(ghost: true))
                    }
                    .padding(.top, 8)
                }
                .padding(40)
            ...
            .overlay(RoundedRectangle(cornerRadius: OopsGlass.radiusWindow, style: .continuous)
