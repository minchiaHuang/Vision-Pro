...
enum WorldArchetype {
    ...
    case ba396Museum    // BA396 exhibition hall (standalone dev world) → bundled USDZ
    ...
    /// absent — see `ParametricWorldBuilder.build` returning nil). `artGallery` and
    /// `ba396Museum` ARE bundled (SpikeAssets).
    var usdzName: String {
        switch self {
